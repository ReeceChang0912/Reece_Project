...
from dico_builder import get_nn_avg_dist
...
def get_word_translation_accuracy(lang1, word2id1, emb1, lang2, word2id2, emb2, method, dico_eval, device=0):
    ...
    for k in [1, 5, 10]:
        ...
        #precision_at_k = 100 * np.mean(list(matching.values())).item()
        #precision_at_k = 100 * torch.mean(torch.stack(list(matching.values())))
       
       
        a=list(matching.values())
        a=list(map(float,a))#把list内的元素变成float型
        
        b=torch.tensor(a)
        b=b.float()
        precision_at_k = 100 * torch.mean(b)
